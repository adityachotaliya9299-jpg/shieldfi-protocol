...
use crate::{WithdrawEvent, InterestAccruedEvent};
...
pub fn withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
    ...
    // ── Phase 1: Accrue interest before health check ──────────────────
    ...
    require!(amount <= capacity, ShieldFiError::WithdrawalRateLimitExceeded);

    require!(position.deposited_amount >= amount, ShieldFiError::WithdrawExceedsDeposit);

    // Health factor check post-withdrawal
    let total_debt = position.borrowed_amount
        .checked_add(position.accrued_interest)
        .ok_or(ShieldFiError::MathOverflow)?;

    if total_debt > 0 {
        let new_deposited = position.deposited_amount
            .checked_sub(amount)
            .ok_or(ShieldFiError::MathOverflow)?;
    ...
    // ── Phase 1: Emit withdraw event ──────────────────────────────────
    emit!(WithdrawEvent {
        user: ctx.accounts.user.key(),
        amount,
        remaining_deposited: position.deposited_amount,
        slot: current_slot,
    });
    // ─────────────────────────────────────────────────────────────────

    msg!("Withdraw: {} | Remaining: {} | Rate: {}/{} | Borrow rate: {}bps",
        amount,
