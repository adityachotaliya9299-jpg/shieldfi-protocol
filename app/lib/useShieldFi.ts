...
export function useShieldFi() {
  ...
  const fetchPositionData = useCallback(async () => {
    ...
    try {
      ...
      const health =
        totalDebt > 0
          ? Math.floor((deposited * cf) / totalDebt)
          : 999999;
  ...
  const deposit = useCallback(
    async (amount: number) => {
      ...
      try {
        ...
        const lamports = new BN(amount * 1_000_000); // 6 decimals
  ...
  const withdraw = useCallback(
    async (amount: number) => {
      ...
      try {
        ...
        const lamports = new BN(amount * 1_000_000);
  ...
  useEffect(() => {
    if (wallet.publicKey) fetchPositionData();
  }, [wallet.publicKey, fetchPositionData]);
